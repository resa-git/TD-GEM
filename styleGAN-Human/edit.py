...
@click.command()
@click.pass_context
@click.option('--network', 'ckpt_path', help='Network pickle filename', default = 'pretrained_models/stylegan2_1024.pkl' , required=True)
@click.option('--attr_name', help='choose one of the attr: upper_length or bottom_length', type=str, required=True, default='upper_length')
@click.option('--trunc', 'truncation', type=float, help='Truncation psi', default=0.8, show_default=True)
@click.option('--gen_video', type=bool, default=False, help='If want to generate video')
@click.option('--combine', type=bool, default=True,  help='If want to combine different editing results in the same frame')
@click.option('--seeds', type=legacy.num_range, help='List of random seeds', default='61531,61570,61571,61610')
...
@click.option('--output_name',  help='output combined file', default = 'final')
...
def main(
    ...
    # ckpt_path =  'outputs/old/ptis_wp/checkpoints/model_seed4000.pth'
    # attr_name =  'upper_length'
    # truncation =  0.8
    # gen_video =  False
    # combine =  False
    # seeds =  [61531, 61570, 61571, 61610]
    # outdir =  'utputs/old/ptis_wp/edit_upper'
    # real =  True
    # real_w_path =  'outputs/old/ptis_wp/embeddings/test/PTI/seed4000/0.pt'
    # real_img_path =  'outputs/old/ptis_wp/images/seed4000.png'
    ## convert pkl to pth
    ...
    with torch.no_grad():
        ...
        legacy.visual(final, os.path.join(outdir, output_name + '.jpg'))
        return 0
