...
from mapper2.datasets.latents_dataset import LatentsDataset, StyleSpaceLatentsDataset
from mapper2.styleclip_mapper import StyleCLIPMapper
from mapper2.training.ranger import Ranger
from mapper2.training import train_utils
from mapper2.training.train_utils import convert_s_tensor_to_list
...
class Coach:
	def __init__(self, opts):
		...
		#with torch.no_grad():
		#		self.net.latent_avg = self.net.latent_avg.to(self.device)
	...
	def train(self):
		...
		while self.global_step < self.opts.max_steps:
			for batch_idx, batch in enumerate(self.train_dataloader):
				...
				# Validation related
				val_loss_dict = None
				if self.global_step >0 and (self.global_step % self.opts.val_interval == 0 or self.global_step == self.opts.max_steps - 1):
					val_loss_dict = self.validate()
					if val_loss_dict and (self.best_val_loss is None or val_loss_dict['loss'] < self.best_val_loss):
						self.best_val_loss = val_loss_dict['loss']
						self.checkpoint_me(val_loss_dict, is_best=True)

				if self.global_step >0 and (self.global_step % self.opts.save_interval == 0 or self.global_step == self.opts.max_steps - 1):
					if val_loss_dict is not None:
						self.checkpoint_me(val_loss_dict, is_best=False)
					else:
						self.checkpoint_me(loss_dict, is_best=False)

				if self.global_step >0 and self.global_step == self.opts.max_steps:
	...
	def configure_datasets(self):
		...
		if self.opts.work_in_stylespace:
			...
			train_dataset_celeba = LatentsDataset(latents='train', opts=self.opts)
			test_dataset_celeba = LatentsDataset(latents='test',opts=self.opts)
