...
from mapper2.datasets.latents_dataset import LatentsDataset
from mapper2.hairclip_mapper import HairCLIPMapper
from mapper2.training.ranger import Ranger
...
class Coach:
	def __init__(self, opts):
		...
		self.maintain_color_for_hairstyle_loss = self.average_color_loss#average_lab_color_loss.AvgLabLoss(self.opts).to(self.device).eval()
	...
	def train(self):
		...
		while self.global_step < self.opts.max_steps:
			for batch_idx, batch in enumerate(self.train_dataloader):
				...
				w, G_path, im_path, hairstyle_text_inputs, color_text_inputs, selected_description_tuple, hairstyle_tensor, color_tensor = batch
				...
				with torch.no_grad():
					self.net.load_weights(G_path[0])
					x, _ = self.net.decoder(w, input_is_latent=True, randomize_noise=False, truncation=1, real=True)
				...
				x_hat, w_hat = self.net.decoder(w_hat, input_is_latent=True, return_latents=True, randomize_noise=False, truncation=1, real=True)
		...
		for batch_idx, batch in enumerate(self.test_dataloader):
			...
			with torch.no_grad():
				...
				if color_tensor.shape[1] != 1:
					...
					color_tensor_hairmasked = torch.Tensor([0]).unsqueeze(0).cuda()
				w_hat = w + 0.1 * self.net.mapper(w, hairstyle_text_inputs, color_text_inputs, hairstyle_tensor_hairmasked, color_tensor_hairmasked)
				x_hat, _ = self.net.decoder(w_hat, input_is_latent=True, randomize_noise=False, truncation=1, real=True)
	...
	def configure_datasets(self):
		...
		train_dataset_celeba = LatentsDataset(latents="train", opts=self.opts, status='train')
		test_dataset_celeba = LatentsDataset(latents="test",   opts=self.opts, status='test')
	...
	def calc_loss(self, w, x, w_hat, x_hat, hairstyle_text_inputs, color_text_inputs, hairstyle_tensor, color_tensor, selected_description):
		...
		if self.opts.text_manipulation_lambda > 0: #Clip loss
			if hairstyle_text_inputs.shape[1] != 1: #hair style (no image, no color, text has value and shape greater than 1)
			...
			if color_text_inputs.shape[1] != 1: #color (no image)
		...
		if self.opts.image_hairstyle_lambda > 0: # This is not used in the project
			if hairstyle_tensor.shape[1] != 1:
		...
		if self.opts.image_color_lambda > 0: # This is not used in the project
			if color_tensor.shape[1] != 1:
		...
		if self.opts.maintain_color_lambda > 0: #color only used for text not color, color shape should be 1 but not text, image is not used (important hairstyle_text_inputs.shape[1] != 1)
			if ((hairstyle_tensor.shape[1] != 1) or (hairstyle_text_inputs.shape[1] != 1)) and (color_tensor.shape[1] == 1) and (color_text_inputs.shape[1] == 1):
				loss_maintain_color_for_hairstyle, loss_background = self.maintain_color_for_hairstyle_loss(x, x_hat)
		...
		if self.opts.background_lambda > 0: #for color since "maintain_color_lambda" only used for text, we need to compute backgound again
			if not (((hairstyle_tensor.shape[1] != 1) or (hairstyle_text_inputs.shape[1] != 1)) and (color_tensor.shape[1] == 1) and (color_text_inputs.shape[1] == 1)):
				_, loss_background = self.maintain_color_for_hairstyle_loss(x, x_hat)
				background_lambda = self.opts.background_lambda_color
			else:
				background_lambda = self.opts.background_lambda
			loss_dict['loss_background'] = float(loss_background)
			loss += loss_background * background_lambda *self.opts.attribute_preservation_lambda
		
		if self.opts.id_lambda > 0: #this code can be inside average_color_loss
			loss_id, sim_improvement = self.id_loss(x_hat * self.average_color_loss.gen_hair_mask(x_hat), x * self.average_color_loss.gen_hair_mask(x))
	...
	def parse_and_log_images(self, x, x_hat, img_tensor, title, selected_description, index=None):
		if index is None:
			...
			prefix = selected_description.replace(' ','_')
			path = os.path.join(self.log_dir, title, f'{str(self.global_step).zfill(5)}-{str(index).zfill(5)}_{prefix}.jpg')
