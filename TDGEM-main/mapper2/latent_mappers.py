...
class HairMapper(Module): 
    ...
    def forward(self, x, hairstyle_text_inputs, color_text_inputs, hairstyle_tensor, color_tensor):
        ...
        if not self.opts.no_fine_mapper:
            if hairstyle_text_inputs.shape[1] != 1 and self.opts.text_fine_mapper:
                 x_fine = self.fine_mapping(x_fine, hairstyle_embedding[:, 8:, :], cut_flag=self.hairstyle_cut_flag)
            else:
