...
from mapper2.datasets.latents_dataset_inference import LatentsDatasetInference
from mapper2.options.test_options import TestOptions
...
def run(test_opts):
	...
	dataset = LatentsDatasetInference(opts=opts)
	...
	for input_batch in tqdm(dataloader):
		if global_i not in range(opts.start_index, opts.end_index):
			...
			continue
		with torch.no_grad():
			w, G_path, im_path, hairstyle_text_inputs_list, color_text_inputs_list, selected_description_tuple_list, hairstyle_tensor_list, color_tensor_list = input_batch
			for i in range(len(selected_description_tuple_list)):
				...
				im_path = os.path.basename(im_path[0])
				...
				torchvision.utils.save_image(couple_output, os.path.join(out_path_results, im_path), normalize=True, scale_each=True, range=(-1, 1), nrow=1)
				#torchvision.utils.save_image(couple_output, fname , normalize=True, scale_each=True, range=(-1, 1), nrow=1)
...
def run_on_batch(inputs, hairstyle_text_inputs, color_text_inputs, hairstyle_tensor_hairmasked, color_tensor_hairmasked, net):
	...
	with torch.no_grad():
		...
		x_hat, w_hat = net.decoder(w_hat, input_is_latent=True, return_latents=True, randomize_noise=False, truncation=1, real=True)
		result_batch = (x_hat, w_hat)
		x, _ = net.decoder(w, input_is_latent=True, return_latents=True, randomize_noise=False, truncation=1, real=True)
