...
class LatentsDatasetInference(Dataset):
	def __init__(self, opts):
		...
		if self.opts.editing_type in ['hairstyle', 'both'] and self.opts.input_type.split('_')[0] == 'text':
			...
			self.hairstyle_list = [single_hairstyle_description for single_hairstyle_description in self.hairstyle_description_list]
	...
	def manipulate_hairstyle(self, index):
		if self.opts.input_type.split('_')[0] == 'text':
			...
			selected_hairstyle_description_list = [single_hairstyle_description for single_hairstyle_description in self.hairstyle_list]
		...
		return self.latents[index], self.gen[index], self.imgs[index], hairstyle_text_embedding_list, color_text_embedding_list, selected_hairstyle_description_list, hairstyle_tensor_list, color_tensor_list
	...
	def manipulater_color(self, index):
		if self.opts.input_type.split('_')[-1] == 'text':
			...
			selected_color_description_list = [single_color_description+'clothes' for single_color_description in self.color_list]
		...
		return self.latents[index], self.gen[index], self.imgs[index], hairstyle_text_embedding_list, color_text_embedding_list, selected_color_description_list, hairstyle_tensor_list, color_tensor_list		


	def manipulater_hairstyle_and_color(self, index):
		returned_latent, gen, img, hairstyle_text_embedding_list, _, selected_hairstyle_description_list, hairstyle_tensor_list, _ = self.manipulate_hairstyle(index)
		_, _, _, _, color_text_embedding_list, selected_color_description_list, _, color_tensor_list = self.manipulater_color(index)		
		...
		return returned_latent, gen, img, hairstyle_text_embedding_final_list, color_text_embedding_final_list, selected_description_list, hairstyle_tensor_final_list, color_tensor_final_list
