...
class LatentsDataset(Dataset):
	...
	def manipulate_hairstyle(self, index):
		...
		if random.random() < self.opts.hairstyle_text_manipulation_prob:
			selected_hairstyle_description = np.random.choice(self.hairstyle_list)
		...
		return self.latents[index], self.gen[index], self.imgs[index], hairstyle_text_embedding, color_text_embedding, selected_description, hairstyle_tensor, color_tensor

	def manipulater_color(self, index):
		hairstyle_text_embedding = torch.Tensor([0])
		hairstyle_tensor = torch.Tensor([0])
		selected_color_description = np.random.choice(self.color_list)+'clothes'
	...
	def manipulater_hairstyle_and_color(self, index):
		returned_latent, gen, img, hairstyle_text_embedding, _, selected_hairstyle_description, hairstyle_tensor, _ = self.manipulate_hairstyle(index)
		_, _, _, _, color_text_embedding, selected_color_description, _, color_tensor = self.manipulater_color(index)
		selected_description = f'{selected_hairstyle_description}-{selected_color_description}'
		return returned_latent, gen, img, hairstyle_text_embedding, color_text_embedding, selected_description, hairstyle_tensor, color_tensor

	def no_editing(self, index):
		return self.latents[index], self.gen[index], self.imgs[index], torch.Tensor([0]), torch.Tensor([0]), 'no_editing', torch.Tensor([0]), torch.Tensor([0])
